...
MOCK_SEO = {
    ...
    "hashtags": [
        "#AITools", "#ArtificialIntelligence", "#Tech2024", "#Productivity", "#OpenAI"
    ],
}
...
class TestSEOOptimizer(unittest.TestCase):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_optimize_seo_returns_required_keys(self, mock_openai_cls, _mock_key):
        """optimize_seo should return a dict with titles, description, tags, hashtags."""
        mock_client = MagicMock()
        mock_openai_cls.return_value = mock_client
        mock_client.chat.completions.create.return_value = MagicMock(
            choices=[MagicMock(message=MagicMock(content=json.dumps(MOCK_SEO)))]
        )
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_titles_is_list_of_10(self, mock_openai_cls, _mock_key):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_tags_has_at_least_30(self, mock_openai_cls, _mock_key):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_hashtags_count_range(self, mock_openai_cls, _mock_key):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_description_is_string(self, mock_openai_cls, _mock_key):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_optimize_seo_saves_file(self, mock_openai_cls, _mock_key):
    ...
    @patch("modules.seo_optimizer.get_openai_key", return_value="fake-key")
    @patch("modules.seo_optimizer.openai.OpenAI")
    def test_optimize_seo_raises_on_api_error(self, mock_openai_cls, _mock_key):
        """optimize_seo should raise RuntimeError on OpenAI API failure."""
